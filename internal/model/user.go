...
type User struct {
	...
	PasswordHash       string `gorm:"type:varchar(255)" json:"-"` // 加密后的密码
	...
	AvatarRelativePath string `gorm:"type:varchar(255)"`
...
func (u *User) AsMaskedData() *UserMasked {
	config := config.GetConfig()

	return &UserMasked{
		User:       *u,
		AvatarPath: config.Web.UploadDirStaticPath + "/" + u.AvatarRelativePath,
	}
}
